...
qpnp_chg_bat_if_batt_pres_irq_handler(int irq, void *_chip)
{
	...
	if(batt_present) {
		...
		//qpnp_chg_enable_irq(&chip->chg_vbatdet_lo);
			...
			#if	defined(CONFIG_MACH_S3VE)
			/* this code is for ESD protection */
...
#if	defined(CONFIG_MACH_S3VE)
#define SUDDEN_TEMP_ERROR	300
static int previous_temp_value = 250;
#endif
...
/* bug-fix to read temperature without battery */
	...
	pr_debug("get_bat_temp: adc_code(%d) physical (%lld)\n",
		results.adc_code, results.physical);

#if	defined(CONFIG_MACH_S3VE)
	/* This code is only for target which has NFC interference to VF and AMUX  */
	if(results.physical - previous_temp_value > SUDDEN_TEMP_ERROR)
	{
		pr_err("%s temp value has got problem suddenly. Current temp value = %d'c, Previous temp value = %d'c \n",
			__func__, results.physical, previous_temp_value);
		results.physical = previous_temp_value;
	}
	else
		previous_temp_value = results.physical;
#endif
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		case POWER_SUPPLY_TYPE_DESK_DOCK:
			if (qpnp_chg_is_usb_chg_plugged_in(chip)) {
				pr_err("%s: deskdock connected + VBUS present \n",__func__);
				new_cable_type = CABLE_TYPE_AC;
			} else {
				pr_err("%s: deskdock connected but VBUS not present \n",__func__);
...
sec_bat_read_dt_props(struct qpnp_chg_chip *chip)
{
	...
	chip->batt_pdata->temp_low_block_event = 0 - chip->batt_pdata->temp_low_block_event;
	...
	chip->batt_pdata->temp_low_block_normal = 0 - chip->batt_pdata->temp_low_block_normal;
	...
	chip->batt_pdata->temp_low_block_lpm = 0 - chip->batt_pdata->temp_low_block_lpm;
        ...
        chip->batt_pdata->event_check = of_property_read_bool(chip->spmi->dev.of_node,
                                        "sec,event-check");

        SEC_BAT_OF_PROP_READ(chip, event_waiting_time, "event-waiting-time", rc, 0);
	chip->batt_pdata->event_waiting_time = chip->batt_pdata->event_waiting_time * 60;
        if (rc)
...
*/
	...
	if (chip->event) {
		...
		chip->temp_low_block = chip->batt_pdata->temp_low_block_normal;
		chip->temp_low_recover = chip->batt_pdata->temp_low_recover_normal;
		pr_debug("all events cleared chip->event(0x%x)\n",chip->event);
		pr_debug("SEC BTM: set normal temperature limits high_block(%d) high_recover(%d)\n",
			chip->temp_high_block,chip->temp_high_recover);
		pr_debug("SEC BTM: set normal temperature limits low_block(%d) low_recover(%d)\n",
			chip->temp_low_block,chip->temp_low_recover);
	}
}
...
static void sec_handle_cable_insertion_removal(struct qpnp_chg_chip *chip)
{
		...
		//if (chip->batt_present) {
			...
			if (chip->ovp_uvlo_state == 0) {
				...
				if (chip->siop_level == 100) {
					...
					pr_err("TA : SIOP change in current ibatmax_ma(0), Set to 50mA\n");
					qpnp_chg_ibatmax_set(chip, QPNP_CHG_IBATMAX_MIN);
				} else {
	...
	/* check OVP/UVLO state */
	if(chip->cable_type != CABLE_TYPE_NONE) {
		if(chip->ovp_uvlo_state == 0) {
			pr_err("cable inserted USB-IN voltage OK (%d)\n",
				chip->ovp_uvlo_state);
		} else {
			pr_err("cable inserted but USB-IN voltage not OK (%d)\n",
				chip->ovp_uvlo_state);
			sec_pm8226_stop_charging(chip);
		}
	}
	...
	wake_lock_timeout(&chip->monitor_wake_lock, 3*HZ);
...
static void sec_bat_monitor(struct work_struct *work)
{
	...
	if (!batt_present) {
		...
		if (chip->batt_status != POWER_SUPPLY_STATUS_DISCHARGING) {
			chip->batt_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
			sec_pm8226_stop_charging(chip);
		}
	}
	...
	if ((chip->cable_type == CABLE_TYPE_USB ||
		...
		(chip->batt_health == POWER_SUPPLY_HEALTH_GOOD)) {
			...
			if (chip->recharging_cnt > 3) {
				sec_pm8226_start_charging(chip);
				chip->is_recharging = true;
				pr_err("Battery re-charging state by battery voltage: recharging count (%d) \n",
						chip->recharging_cnt);
			}
			#endif
	}
	...
	for(i=0;i<6;i++)
	{
		...
		pr_err("Debug Temp for Sensor:%d --- %ld\n",i,temp);
	}
	//power_supply_changed(chip->usb_psy);
	power_supply_changed(&chip->batt_psy);
...
static void sec_ovp_uvlo_worker(struct work_struct *work)
{
	 ...
	 */
	if(chip->cable_type == CABLE_TYPE_NONE) {
		pr_err("OVP/UVLO check: no cable connected, reset status\n");
		/* SW work around for UVLO trigger when no cable present */
		chip->batt_health = POWER_SUPPLY_HEALTH_GOOD;
		chip->batt_status = POWER_SUPPLY_STATUS_DISCHARGING;
	} else {
                        ...
                        struct qpnp_chg_chip *chip, int raw_soc)
{
        ...
        scaled_soc = (raw_soc < chip->batt_pdata->capacity_min) ?
                ...
                (chip->batt_pdata->capacity_max - chip->batt_pdata->capacity_min));
